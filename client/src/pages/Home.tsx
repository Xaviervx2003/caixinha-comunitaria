...
export default function Home() {
  ...
  const { data: allTransactions = [] } = trpc.caixinha.getAllTransactions.useQuery(
    undefined
  ) as { data: Transaction[] };

  const { data: auditLogEntries = [] } = trpc.caixinha.getAuditLog.useQuery(
    { limit: 50 }
  ) as { data: AuditEntry[] };
  ...
  const isCurrentMonthClosed = useMemo(
    () => monthlyHistory.some((s) => s.month === currentMonthKey),
  ...
  const { totalDebts, totalFees, totalInterest } = useMemo(() => {
    const debts = participants.reduce((acc, p) => acc + parseFloat(p.currentDebt.toString()), 0);
    ...
    return { totalDebts: debts, totalFees: cols.totalFees, totalInterest: cols.totalInterest };
  }, [participants, allTransactions, participantRoles]);

  const debtors = useMemo(
    () => participants.filter((p) => parseFloat(p.currentDebt.toString()) > 0).length,
    [participants]
  );
  ...
  const handleAddParticipant = async () => {
    ...
    try {
      ...
      setNewParticipantName(''); setNewParticipantEmail(''); setNewParticipantLoan(''); setNewParticipantRole('member');
  ...
  const handleAmortize = async () => {
    ...
    try {
      ...
      showSuccessToast('Amortização registrada!');
    } catch { showErrorToast('Erro'); }
  };
    ...
    <div className="flex h-screen bg-[#F4F5F7] overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            <div className="max-w-7xl mx-auto">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  <div className="bg-gray-100 rounded-full p-6 mb-4"><Users className="w-12 h-12 text-gray-400" /></div>
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
                    <div className="flex items-center gap-3 w-full sm:w-auto">
                      <div className="bg-[#00C853]/10 text-[#00C853] p-2 rounded-lg"><Users className="w-5 h-5" /></div>
                      <div>
          ...
          {activeSection === 'transacoes' && (
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="divide-y divide-gray-50">
                  {allTransactions.length === 0 ? (
                    ...
                    allTransactions.slice().reverse().map((t) => {
                      ...
                      const isPayment = t.type === 'payment'; const isAmort = t.type === 'amortization'; const isLoan = t.type === 'loan';
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100"><h2 className="font-bold text-gray-800">Log de Auditoria</h2></div>
                <div className="divide-y divide-gray-50">
              ...
              <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <input type="text" value={settingsName} onChange={(e) => setSettingsName(e.target.value)} placeholder="Ex: Caixinha do Trabalho"
      ...
      <Dialog open={isPaymentOpen} onOpenChange={setIsPaymentOpen}>
        <DialogContent className="bg-white rounded-xl border-0 shadow-2xl w-full sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2"><Label className="font-bold text-sm">Data Real do Pagamento</Label><Input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} className="border-2 rounded-lg h-11 font-medium" /></div>
            <div className="grid gap-2">
      ...
      <Dialog open={isHistoryOpen} onOpenChange={setIsHistoryOpen}>
        <DialogContent className="bg-white rounded-xl border-0 shadow-2xl w-full sm:max-w-[500px]">
          ...
          <div className="py-4 space-y-6 max-h-[60vh] overflow-y-auto">
            {selectedParticipant && (
              <>
                {participantStatementQuery.isLoading ? <p className="text-sm text-gray-500">Carregando extrato...</p> : (
                  <TransactionHistory transactions={(participantStatementQuery.data?.transactions || []) as any} />
                )}
