  ...
  caixinha: router({
    ...
    addParticipant: protectedProcedure
      .input(
        z.object({
          ...
          totalLoan: z.number().nonnegative().max(999999.99).default(0),
        })
      )
    ...
    registerPayment: protectedProcedure
      .input(
        z.object({
          ...
          quotaMultiplier: z.number().int().min(1).max(12).default(1),
        })
      )
      .mutation(async ({ input, ctx }) => {
        const db = await getDb();
        ...
        return db.transaction(async (tx) => {
          ...
          const quotaMultiplier = input.quotaMultiplier ?? 1;
 
          // Usa a data fornecida (retroativo) ou hoje para calcular multa
          const paymentDate = input.paymentDate
            ? new Date(input.paymentDate + 'T12:00:00')
            : new Date();
          const paidLate = isLatePayment(input.month, paymentDate, caixinha.paymentDueDay ?? 5);
 
          const currentDebt = new Decimal(p.currentDebt);
          const { quota, interest, total } = paidLate
            ? calcLateMonthlyPayment(currentDebt, role, quotaMultiplier)
            : calcMonthlyPayment(currentDebt, role, quotaMultiplier);
          ...
          if (existingPayment.length > 0) {
            await tx
              ...
              .set({ paid: true, paidLate: paidLate, paidAt: new Date() })
          ...
          const quotaLabel = quotaMultiplier > 1
            ? `${quotaMultiplier}x Cotas R$ ${quota.toFixed(2)}`
            : `Cota R$ ${quota.toFixed(2)}`;
          const lateLabel = paidLate ? ' + Multa' : '';
          const desc = `${quotaLabel} + Juros R$ ${interest.toFixed(2)}${lateLabel}`;
          ...
          await tx.insert(auditLog).values({
            ...
            year: input.year,
            amount: total.toFixed(2),
            description: `Pagamento registrado: ${desc} = R$ ${total.toFixed(2)}`,
          });
 
          return { success: true, total: total.toFixed(2), paidLate };
        });
      }),
    ...
    updateParticipantLoan: protectedProcedure
      .input(
        z.object({
          participantId: participantIdSchema,
          newTotalLoan: z.number().nonnegative().max(999999.99),
        })
      )
      .mutation(async ({ input, ctx }) => {
        const db = await getDb();
    ...
    updateParticipantDebt: protectedProcedure
      .input(
        z.object({
          participantId: participantIdSchema,
          newCurrentDebt: z.number().nonnegative().max(999999.99),
        })
      )
      .mutation(async ({ input, ctx }) => {
        const db = await getDb();
